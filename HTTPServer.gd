...
## Emite when the thread is released.
signal thread_released()
	...
	func _init(environ):
		...
		if content_type[0] == "application/x-www-form-urlencoded":
			for key_value in environ["CONTENT"].split("&"):
				form_data[key_value.split("=")[0].uri_decode()] = key_value.split("=")[1].uri_decode()
...
class StreamPeerHTTP:
	signal stream_peer_http_killed(stream_peer_http)
	...
	func _init(settings_parameter):
		...
		timeout = settings.keep_alive_timeout + Time.get_unix_time_from_system()
	...
	func process():
		if((settings.keep_alive and Time.get_unix_time_from_system() < timeout) or not settings.keep_alive or header) and settings.peer.is_connected_to_host():
			if peer.get_status() == StreamPeerSSL.STATUS_CONNECTED or peer.get_status() == StreamPeerSSL.STATUS_HANDSHAKING:
				if not header:
					...
					if http_response.ends_with("\r\n"):
						...
						if result:
							...
							if environ["SERVER_PROTOCOL"] == "HTTP/1.1" and not "HOST" in environ:
								environ = {}
						else:
							print("erreur, 400")
						...
						if response[0] == OK:
							if environ:
								...
								if typeof(data_vus) != TYPE_ARRAY:
									print("(HTTPServer) The function assigned to the \"application\" variable of the server must return an \"Array\" type object.")
									data_vus = http_error(environ, 500)
								elif typeof(data_vus[0]) != TYPE_STRING:
									print("(HTTPServer) The object at index 0 of the \"Array\" object returned by the function assigned to the \"application\" variable of the server must be of type \"String\".")
									data_vus = http_error(environ, 500)
								elif typeof(data_vus[1]) != TYPE_DICTIONARY:
									print("(HTTPServer) The object at index 1 of the \"Array\" object returned by the function assigned to the \"application\" variable of the server must be of type \"Dictionary\".")
									data_vus = http_error(environ, 500)
								elif typeof(data_vus[2]) != TYPE_RAW_ARRAY:
									print("(HTTPServer) The object at index 2 of the \"Array\" object returned by the function assigned to the \"application\" variable of the server must be of type \"PoolByteArray\".")
									data_vus = http_error(environ, 500)
								
								if "RANGE" in environ and environ["REQUEST_METHOD"] == "GET":
									regex.compile("^bytes=(\\d+)-(\\d*)")
									var resultTTT = regex.search(environ["RANGE"])
						...
						var datetime_datas = [
							DAYS_NAME[datetime.weekday],
							str(datetime.day).pad_zeros(2),
							MONTHS_NAME[datetime.month],
							datetime.year,
						...
						if verboss_mode:
							prints(environ["REMOTE_ADDR"] + ":" + str(environ["REMOTE_PORT"]), environ["PATH"], environ["REQUEST_METHOD"], data_vus[0])
				...
				if header:
					...
					if error_and_cursor_responce[0]:
						...
						if header.size() != error_and_cursor_responce[1]:
							header = header.subarray(error_and_cursor_responce[1], -1)
						else:
							header = null
					
					# Quand la totalité de la réponce a été servie.
					if not header:
						data_vus = []
						print("complet")
						http_response = ""
